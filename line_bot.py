                        ...
                        "contents": [
                            {
                                ...
                                "action": {
                                    "type": "postback",
                                    "label": "GO！",
                                    "data": json.dumps({"action": "search_start"}),
                                    "displayText": "GO！",
                                },
                                "style": "link",
                                "color": "#905c44",
...
def safe_get_text(value, field_name=""):
    ...
    if isinstance(value, str):
        return (
            value.replace("有訊號", "").strip()
            if field_name == "signal_strength"
            else value
        )
...
def create_camp_bubble(camp):
    ...
    bubble = {
        ...
        "hero": {
            ...
            "url": (
                camp["image_urls"][0]
                if isinstance(camp.get("image_urls"), list) and camp["image_urls"]
                else "https://example.com/default.jpg"
            ),
            "size": "full",
            "aspectRatio": "20:13",
            "aspectMode": "cover",
...
def handle_message(event, Campsite):
    ...
    if (
        message_text == "開始搜尋！"
        or message_text == "開始搜尋"
        or message_text == "開始"
        or message_text == "搜尋"
        or message_text == "搜尋營地"
        or message_text == "搜尋營地！"
        or message_text == "GO!"
        or message_text == "go"
    ):
...
例如：
   ...
   - 北部 可帶寵物""",
    ...
    return handle_search_results(
        event["replyToken"], campsites, current_page, message_text
    )
    ...
    try:
        ...
        if data.get("action") == "next_page":
            try:
                ...
                logger.info(
                    f"處理下一頁請求: 頁碼={current_page}, 關鍵字={keyword}"
                )  # 添加日誌
                ...
                # 處理搜尋結果
                return handle_search_results(
                    event["replyToken"], campsites, current_page, keyword
                )
            except Exception as e:
        ...
        # 處理搜尋開始
            ...
            if (
                not state
                or "city" not in state
                or not state.get("city")
                        ...
                        [
                            {
                                ...
                                + (
                                    f"- 海拔：{state['altitude']}\n"
                                    if state.get("altitude")
                                    and state["altitude"] != "兩者皆可"
                                    else ""
                                )
                                + (
                                    f"- 寵物：{state['pet']}\n"
                                ...
                                + (
                                    f"- 停車：{state['parking']}"
                                    if state.get("parking")
                                    and state["parking"] != "兩者皆可"
                                    else ""
                                )
                                + "\n請嘗試放寬搜尋條件。",
                            }
...
def handle_search_results(reply_token, campsites, current_page, keyword):
    ...
    try:
        ...
        if current_page < total_pages:
            next_page_bubble = create_next_page_bubble(
                current_page, total_pages, keyword
            )
        ...
        flex_message = {"type": "flex", "altText": "營區搜尋結果", "contents": carousel}
